...
class _ProteinPageState extends State<ProteinPage> {
  ...
  Future<void> fetchProducts() async {
    try {
      var response = await http.get(Uri.parse('http://13.125.255.90:8080/api/product-list/protein'));
      if (response.statusCode == 200) {
        setState(() {
          products = json.decode(utf8.decode(response.bodyBytes));
        });
      } else {
        throw Exception('Failed to load protein products with status: ${response.statusCode}');
      }
    } catch (e) {
      print('Error fetching protein products: $e');
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            const Text('Protein', style: TextStyle(fontSize: 20, fontFamily: 'mainfont', color: Colors.black)),
            Container(
              height: 35,
