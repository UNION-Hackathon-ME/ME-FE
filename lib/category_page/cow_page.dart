...
class BeefPage extends StatefulWidget {
  const BeefPage({Key? key}) : super(key: key);
...
class _BeefPageState extends State<BeefPage> {
  ...
  Future<void> fetchProducts() async {
    try {
      var response = await http.get(Uri.parse('http://13.125.255.90:8080/api/product-list/beef'));
      if (response.statusCode == 200) {
        setState(() {
          products = json.decode(response.body);
        });
      } else {
        throw Exception('Failed to load beef products with status: ${response.statusCode}');
      }
    } catch (e) {
      print('Error fetching beef products: $e');
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            const Text('Beef', style: TextStyle(fontSize: 20, fontFamily: 'mainfont', color: Colors.black)),
            Container(
              height: 35,
