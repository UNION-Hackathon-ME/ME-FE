...
class PorkPage extends StatefulWidget {
  const PorkPage({Key? key}) : super(key: key);
  ...
  Future<void> fetchProducts() async {
    try {
      var response = await http
          .get(Uri.parse('http://13.125.255.90:8080/api/product-list/pork'));
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to load pork products with status: ${response.statusCode}');
      }
    } catch (e) {
      print('Error fetching pork products: $e');
    }
  }
