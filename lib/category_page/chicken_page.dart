...
class _ChickenPageState extends State<ChickenPage> {
  ...
  Future<void> fetchProducts() async {
    try {
      var response = await http.get(Uri.parse('http://13.125.255.90:8080/api/product-list/chicken'));
      if (response.statusCode == 200) {
        setState(() {
          products = json.decode(utf8.decode(response.bodyBytes));
        });
      } else {
        throw Exception('Failed to load chicken products with status: ${response.statusCode}');
      }
    } catch (e) {
      print('Error fetching chicken products: $e');
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            const Text('Chicken', style: TextStyle(fontSize: 20, fontFamily: 'mainfont', color: Colors.black)),
            Container(
              height: 35,
